  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-4 lg:px-4 py-4 md:py-12">
      <div className="text-center mb-6">
        <h1 className="text-lg md:text-xl font-comic text-black mb-4 inline-block bg-cyan-400 border-2 border-black px-4 py-2 shadow-[2px_2px_0px_0px_#000] rotate-1">
            RESULTADOS
        </h1>
        <p className="text-black font-bold text-lg md:text-xl max-w-2xl mx-auto mt-2 bg-white p-3 border-2 border-black inline-block transform -rotate-1 shadow-[2px_2px_0px_0px_#000]">Transparencia total. Aquí puedes ver todos nuestros sorteos pasados.</p>
      </div>

      {loading ? (
        <div className="p-20 text-center font-comic text-lg">CARGANDO RESULTADOS...</div>
      ) : raffles.length === 0 ? (
        <div className="bg-white rounded-xl p-4 text-center border-2 border-black shadow-[2px_2px_0px_0px_#000] max-w-2xl mx-auto transform rotate-1">
          <div className="w-20 h-20 bg-yellow-300 border-2 border-black rounded-full flex items-center justify-center mx-auto mb-6 shadow-[2px_2px_0px_0px_#000]">
            <Trophy className="w-10 h-10 text-black" />
          </div>
          <h2 className="text-xl font-comic text-black mb-2">Aún no hay sorteos finalizados</h2>
          <p className="text-black font-bold text-lg">Próximamente publicaremos aquí a nuestros primeros ganadores.</p>
        </div>
      ) : (
        <div className="space-y-10 max-w-4xl mx-auto">
          {raffles.map((raffle) => (
            <div key={raffle.id} className="bg-white rounded-xl shadow-[2px_2px_0px_0px_#000] border-2 border-black overflow-hidden md:flex items-center hover:translate-x-1 hover:translate-y-1 hover:shadow-[2px_2px_0px_0px_#000] transition-all">
              ...
              <div className="p-4 md:w-2/3 flex flex-col justify-between">
                <div>
                  <h3 className="text-xl font-comic text-black mb-2">{raffle.title}</h3>
                ...
                <div className="space-y-4">
                  {(raffle.winners && raffle.winners.length > 0) ? (
                      ...
                      {raffle.winners.sort((a: any, b: any) => (a.position || 0) - (b.position || 0)).map((w: any) => {
                        ...
                        return (
                          <div 
                            ...
                            className={`border-2 border-black rounded-xl overflow-hidden flex flex-col sm:flex-row shadow-[2px_2px_0px_0px_#000] transition-all hover:scale-[1.01] ${
                              isFirst 
                          ...
                          >
                            ...
                            <div className={`p-4 sm:p-4 flex-grow flex flex-col justify-center ${isFirst ? 'sm:p-4' : ''}`}>
                              ...
                              <div className="flex items-center justify-between sm:justify-start gap-4 flex-wrap">
                                  <div className="relative">
                                    <p className={`font-comic text-red-500 drop-shadow-[2px_2px_0px_#fff] ${isFirst ? 'text-xl sm:text-lg' : 'text-xl sm:text-lg'}`}>#{w.ticketNumber}</p>
                                    <div className="absolute -top-1 -right-1 bg-black text-white text-[10px] px-1 rounded font-bold">№</div>
                                  </div>
                                  <p className={`font-black uppercase bg-white border-2 border-black shadow-[2px_2px_0px_0px_#000] ${isFirst ? 'text-xl sm:text-lg px-4 py-2' : 'text-sm px-4 py-1'}`}>
                                    {maskName(w.userName)}
                                  </p>
                              </div>
                            </div>
                    ...
                    <div className="bg-yellow-200 border-2 border-black rounded-2xl p-4 flex justify-between items-center shadow-[2px_2px_0px_0px_#000] transform rotate-1">
                      <div>
                        <p className="text-sm text-black font-bold uppercase tracking-wider mb-1">TICKET GANADOR</p>
                        <p className="font-comic text-lg text-red-500 drop-shadow-[2px_2px_0px_#fff]">
                  ...
                  <div className="flex justify-end pt-2">
                    <button className="flex items-center space-x-2 text-white bg-red-500 border-2 border-black px-4 py-2 rounded-xl hover:bg-black transition-colors font-bold shadow-[2px_2px_0px_0px_#000] hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none text-sm">
